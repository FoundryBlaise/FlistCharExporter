...
async function sidecarFetch(path, { method = 'GET', body = null, requireAuth = true } = {}) {
  const headers = {};
  ...
  try {
    res = await fetch(SIDECAR_BASE + path, {
      method,
      headers,
      body: body && typeof body === 'object' && !(body instanceof ArrayBuffer) && !(body instanceof Blob)
        ? JSON.stringify(body)
        : body,
    });
  } catch (err) {
...
async function beginHandshake() {
  const res = await sidecarFetch('/restore/handshake', { method: 'POST', requireAuth: false });
  if (res.ok === false) return res;
  if (!res.ok) return { ok: false, status: res.status, error: 'handshake_failed' };
    ...
    try {
      switch (msg.type) {
        ...
        case 'list_snapshots': {
          ...
          if (res.ok === false) { sendResponse(res); return; }
          if (res.status === 401) {
            await clearToken();
          ...
          if (!res.ok) {
            sendResponse({ ok: false, status: res.status, error: 'list_failed' });
            return;
          }
        ...
        case 'fetch_snapshot': {
          ...
          if (res.ok === false) { sendResponse(res); return; }
          if (!res.ok) {
            sendResponse({ ok: false, status: res.status, error: 'fetch_failed' });
            return;
          }
        ...
        case 'snapshot_form_state': {
          ...
          if (res.ok === false) { sendResponse(res); return; }
          if (!res.ok) {
            sendResponse({ ok: false, status: res.status, error: 'snapshot_failed' });
            return;
          }
